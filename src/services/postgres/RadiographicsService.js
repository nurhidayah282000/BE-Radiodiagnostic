...
class RadiographicsService {
  ...
  async addRadiographic(panoramikPicture, patientId, doctorId, radiographerId) {
    ...
    const historyQuery = {
      text: `INSERT INTO histories (id, patient_id, doctor_id, radiographer_id, radiographic_id)
        VALUES($1, $2, $3, $4, $5) RETURNING id`,
      values: [historyId, patientId, doctorId, radiographerId, radiographicId],
    };
  ...
  async getAllRadiographics(month) {
    ...
    INNER JOIN users doctor ON histories.doctor_id = doctor.id
  ...
  async editRadiographicInterpretation(radiographicId, { manualInterpretation }) {
    ...
    if (!result.rowCount) {
      throw new InvariantError('Interpretasi manual radiografi gagal diperbarui');
    }
    return result.rows[0];
  }
